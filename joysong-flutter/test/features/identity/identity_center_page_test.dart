...
void main() {
  ...
  testWidgets('consultant affiliation entry respects read-only access',
    ...
    expect(find.text('机构归属与申请'), findsOneWidget);
    expect(find.text('申请加入机构'), findsNothing);
    expect(find.text('机构归属'), findsNothing);

    await tester.tap(find.text('机构归属与申请'));
    await tester.pumpAndSettle();

    expect(find.text('当前账号仅可查看机构归属记录'), findsOneWidget);
    expect(
  ...
  testWidgets('catalog entry keeps the selected identity scope',
    ...
    await tester.tap(find.text('专业目录'));
    await tester.pumpAndSettle();
    ...
    final doctorGroup = find.byKey(const ValueKey('management-group-doctor'));
    await tester.tap(doctorGroup);
    await tester.pumpAndSettle();
    await tester.tap(
      find.descendant(of: doctorGroup, matching: find.text('专业目录')),
    );
    await tester.pumpAndSettle();
