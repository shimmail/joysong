...
class HelpAndFeedbackPage extends StatefulWidget {
  const HelpAndFeedbackPage({this.onCustomerService, super.key});
...
class _HelpAndFeedbackPageState extends State<HelpAndFeedbackPage> {
  ...
  static const _faqs = <(String, String)>[
    ('如何查看预约进度？', '进入「我的订单」，可按状态查看预约、核销和退款进度。'),
    ('验证码收不到怎么办？', '请检查国家/地区代码与手机号，并在倒计时结束后重试。'),
    ('怎样保护隐私材料？', '身份认证材料只通过私有认证接口上传，不会进入公开日记或图片接口。'),
    ('如何申请退款？', '打开订单详情，在当前状态允许时选择「申请退款」并填写原因。'),
  ];
            ...
            Card(
              child: ExpansionTile(
                ...
                onExpansionChanged: (expanded) =>
                    setState(() => _expanded = expanded ? index : null),
                title: Text(_faqs[index].$1),
                childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
                children: [
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text(_faqs[index].$2),
                  ),
                ],
              ),
            ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('反馈提交接口尚未开放，可通过客服会话留言，避免意见内容被虚假标记为已提交。'),
                  const SizedBox(height: 14),
                  FilledButton.icon(
                    onPressed: widget.onCustomerService,
