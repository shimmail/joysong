...
- `docs/FLUTTER_API_CONTRACT.md`：新增顾问订单端点、字段、阶段和错误码。
- 顾问完整链路手工测试：增加列表、详情、越权、身份失效和会话只读场景。
- MVP 功能状态：顾问工作台实现并通过验收后再解除阻断。
- `design/TRAVEL_GROUND_SERVICE_PAYMENT_WORKFLOW.puml`：补充顾问读取订单和进入订单会话的路径。

UML 只维护源码，图片仍由项目维护者手动生成。
