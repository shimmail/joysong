...
**Files:**
- Create: `joysong-server/src/main/resources/db/migration/V17__add_revenue_ledger.sql`
...
Add a test that reads `V17__add_revenue_ledger.sql` and asserts these exact fragments exist:
...
Expected: FAIL because `V17__add_revenue_ledger.sql` does not exist.
...
```powershell
git add joysong-server/src/main/resources/db/migration/V17__add_revenue_ledger.sql joysong-server/src/main/resources/db/migration/B1__init_schema.sql joysong-server/src/test/kotlin/com/joysong/server/identity/service/DoctorInstitutionChangeMigrationTest.kt
