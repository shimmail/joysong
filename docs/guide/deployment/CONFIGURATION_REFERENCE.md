...
2026-09-06 临时样例挂载期间，已通过独立覆盖文件启用地接服务费模拟支付，自动付款仍关闭；AI/翻译实测受百炼 `Arrearage` 账户错误阻塞。当前状态、覆盖文件撤回及正式发布前的处理顺序见 [临时挂载与功能启用记录](TEMPORARY_CATALOG.md#2026-09-06-ai-与地接模拟支付)。
