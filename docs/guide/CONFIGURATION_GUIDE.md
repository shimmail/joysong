...
2. 在服务器的环境变量页面新增：
   ...
   # 仅空数据库首次创建管理员时临时设置；已有管理员的正常重启请不要设置
...
3. 仅在空数据库首次启动服务端时临时设置 `ADMIN_PASSWORD`，用于创建 `ADMIN_PHONE` 指定的唯一固定管理员。确认能访问 `https://你的API域名/actuator/health` 后，删除 `ADMIN_PASSWORD`。已有管理员的正常重启既不要求也不使用该变量，配置不会重置现有管理员密码。
...
4. SSE 流式回答随应用固定启用，不新增环境变量或流式开关；当前仅支持 `AI_AGENT_PROVIDER=qwen`。
...
- `JWT_SECRET` 随机且稳定。`ADMIN_PASSWORD` 只用于空库首次创建唯一固定管理员；初始化完成后移除，已有管理员的正常重启不得要求或使用它，且配置永不重置现有密码。
