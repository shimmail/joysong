...
class DoctorInstitutionChangeMigrationTest {
    ...
    @Test
    fun `V20 repairs doctor project price after merged V12 drift`() {
        val migration = requireNotNull(
            javaClass.getResource("/db/migration/V20__reconcile_merged_professional_schema.sql")
    ...
    @Test
    fun `V21 revenue ledger migration remains the immutable base schema`() {
    ...
    @Test
    fun `V22 extends revenue ledger snapshots without rewriting V21`() {
        val extension = migration("db/migration/V22__extend_revenue_ledger_snapshots.sql")
