...
CREATE TABLE `account_deletion_requests` (
    ...
    UNIQUE KEY `uk_account_deletion_request_authorization` (`authorization_hash`),
    KEY `idx_account_deletion_requests_user_status` (`user_id`, `status`),
    KEY `idx_account_deletion_requests_expiry` (`request_expires_at`),
...
CREATE TABLE `user_media_assets` (
    ...
    `delete_status` VARCHAR(32) NOT NULL DEFAULT 'ACTIVE',
